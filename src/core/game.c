 ...
 */
{
    ...
    {
        if (iArray[x] == iElement && iElement != 0)
        {
            return x;
        }
    }
 ...
 */
{
    for (int row = 0; row < 3; row++)
        for (int col = 0; col < 3; col++)
        {
            ...
            if (iGameData[row + iBoxStartRow][col + iBox_Start_Col] == iElement && iElement != 0)
            {
                return 1;
            }
        }
 ...
 */
{
    ...
    for (int _x = 0; _x < 9; _x++)
    {
        ...
        {
            time(&end_t);
            dDiff_t = difftime(end_t, start_t);
            if (dDiff_t > 2)
            {
 ...
 */
{
    int iSeconds = iUserTime % 60;
    int iMinutes = iUserTime / 60;
    ...
    sprintf(cStringTime, "%cS%d:%cS%d", cM, iMinutes, cS, iSeconds);
    cStringTime[5] = '\0';
}
